...
async function processChunksParallel(
    ...
    for (let i = 0; i < chunks.length; i += MAX_PARALLEL) {
        ...
        console.log(`   📦 Батч ${batchNum}/${totalBatches} (${batch.length} чанкаў)...`);
...
async function processChunksSequential(
    ...
    for (const chunk of chunks) {
        if (chunk.type !== 'code') {
            process.stdout.write(`   📝 Чанк ${chunk.id + 1}/${chunks.length}...`);
        }
...
async function analyzerNode(state: MDReviewStateType): Promise<Partial<MDReviewStateType>> {
    console.log(`\n${'─'.repeat(50)}`);
    ...
    console.log(`📦 Чанкаў: ${chunks.length} (${textChunks} тэкст, ${codeChunks} код)`);
    console.log(`${getModeEmoji(state.mode)} Рэжым: ${getModeName(state.mode)}`);
    console.log(`⚡ Паралельна: ${state.useParallel ? 'Так' : 'Не'}`);
    
    return { chunks };
}

async function reviewerNode(state: MDReviewStateType): Promise<Partial<MDReviewStateType>> {
...
async function finalizerNode(state: MDReviewStateType): Promise<Partial<MDReviewStateType>> {
    console.log(`\n${'═'.repeat(50)}`);
...
function writeMDFile(originalPath: string, content: string, mode: ReviewMode): string {
...
async function reviewMDFile(
    ...
    const result = await app.invoke(
        ...
        { configurable: { thread_id: `md-v2-${Date.now()}` } }
    );
    ...
    if (result.currentContent !== content) {
        outputPath = writeMDFile(absolutePath, result.currentContent, options.mode);
    }
    ...
    console.log(`${getModeEmoji(options.mode)} Рэжым: ${getModeName(options.mode)}`);
